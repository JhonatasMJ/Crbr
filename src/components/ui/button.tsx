...
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-md border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-1 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-1 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 cursor-pointer",
  {
    variants: {
      variant: {
        default: "bg-yellow-base text-black hover:bg-yellow-dark font-semibold",
        outline:
          "border-border bg-background hover:bg-muted hover:text-foreground aria-expanded:bg-muted aria-expanded:text-foreground dark:border-input dark:bg-input/30 dark:hover:bg-input/50",
        secondary:
          "bg-black text-white hover:bg-blackLight aria-expanded:bg-black aria-expanded:text-white",
        ghost:
...
const buttonIconVariants = cva(
  "inline-flex shrink-0 items-center justify-center rounded-sm [&_svg:not([class*='size-'])]:size-3.5",
  {
    variants: {
      variant: {
        default: "bg-black text-yellow-base",
        outline: "bg-foreground text-background",
        secondary: "bg-yellow-base text-black",
        ghost: "bg-muted text-foreground",
        destructive: "bg-destructive text-white",
        link: "bg-transparent text-current",
      },
      size: {
        default: "size-6",
        xs: "size-4 [&_svg:not([class*='size-'])]:size-2.5",
        sm: "size-5 [&_svg:not([class*='size-'])]:size-3",
        lg: "size-6 [&_svg:not([class*='size-'])]:size-4",
